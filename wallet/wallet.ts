...
import {CryptixAPI, ApiError} from './api';
...
const SompiPerCryptix = 100_000_000

// MaxSompi is the maximum transaction amount allowed in sompi.
const MaxSompi = 1_161_000_000 * SompiPerCryptix

export {cryptixcore, COMPOUND_UTXO_MAX_COUNT, CONFIRMATION_COUNT, COINBASE_CFM_COUNT};
...
class Wallet extends EventTargetImpl {
	...
	static cryptixcore=cryptixcore;
	...
	//cryptixcore.Transaction.EstimatedStandaloneMassWithoutInputs;

	// TODO - integrate with Cryptixcore-lib
	static networkTypes: Object = {
		cryptix: { port: 18110, network: 'cryptix', name : 'mainnet' },
		cryptixtest: { port: 18210, network: 'cryptixtest', name : 'testnet' },
		cryptixsim: { port: 18510, network: 'cryptixsim', name : 'simnet' },
		cryptixdev: { port: 18610, network: 'cryptixdev', name : 'devnet' }
	}
	...
	static CYTX(v:number): string {
		return CYTX(v);
	}


	static initRuntime() {
		return cryptixcore.initRuntime();
	}
	...
	HDWallet: cryptixcore.HDPrivateKey;
	 ...
	 * Current network.
	 */
	network: Network = 'cryptix';

	api: CryptixAPI; //new CryptixAPI();
	...
	constructor(privKey: string, seedPhrase: string, networkOptions: NetworkOptions, options: WalletOptions = {}) {
		super();
		this.logger = CreateLogger('CryptixWallet');
		this.api = new CryptixAPI();
		//@ts-ignore
		...
		if (privKey && seedPhrase) {
			this.HDWallet = new cryptixcore.HDPrivateKey(privKey);
			this.mnemonic = seedPhrase;
		} else {
			const temp = new Mnemonic(Mnemonic.Words.ENGLISH);
			this.mnemonic = temp.toString();
			this.HDWallet = new cryptixcore.HDPrivateKey(temp.toHDPrivateKey().toString());
		}
	...
	*/
	 ...
	 * @param obj
	 * @param obj.toAddr To address in cashaddr format (e.g. cryptixtest:qq0d6h0prjm5mpdld5pncst3adu0yam6xch4tr69k2)
	 * @param obj.amount Amount to send in sompis (100000000 (1e8) sompis in 1 CYTX)
	...
	composeTx({
		...
		try {
			const tx: cryptixcore.Transaction = new cryptixcore.Transaction()
				.from(utxos)
				.to(toAddr, amount)
				.setVersion(0)
				.fee(fee)
				.change(changeAddr)
			if(!skipSign)
				tx.sign(privKeys, cryptixcore.crypto.Signature.SIGHASH_ALL, 'schnorr');
	...
	validateAddress(addr:string):boolean{
		let address = new cryptixcore.Address(addr);
	...
	*/
	 ...
	 * @param txParams
	 * @param txParams.toAddr To address in cashaddr format (e.g. cryptixtest:qq0d6h0prjm5mpdld5pncst3adu0yam6xch4tr69k2)
	 * @param txParams.amount Amount to send in sompis (100000000 (1e8) sompis in 1 CYTX)
	...
	async composeTxAndNetworkFeeInfo(txParamsArg: TxSend): Promise < TxInfo >{
		...
		this.logger.info(`tx ... amount: ${CYTX(txParamsArg.amount)} user fee: ${CYTX(txParamsArg.fee)} max data fee: ${CYTX(txParamsArg.networkFeeMax||0)}`)
		...
		this.logger.verbose(`tx ... need data fee: ${CYTX(dataFee)} total needed: ${CYTX(amountRequested+dataFee)}`)
		this.logger.verbose(`tx ... available: ${CYTX(amountAvailable)} in ${data.utxos.length} UTXOs`)

		if(networkFeeMax && dataFee > networkFeeMax) {
			throw new Error(`Fee max is ${networkFeeMax} but the minimum fee required for this transaction is ${CYTX(dataFee)} CYTX`);
		}
		...
		if(calculateNetworkFee){
			do {
				...
				this.logger.verbose(`tx ... need data fee: ${CYTX(dataFee)} for ${data.utxos.length} UTXOs`);
			...
			if(networkFeeMax && txParams.fee > networkFeeMax)
				throw new Error(`Maximum network fee exceeded; need: ${CYTX(dataFee)} CYTX maximum is: ${CYTX(networkFeeMax)} CYTX`);

		}else if(dataFee > priorityFee){
			throw new Error(`Minimum fee required for this transaction is ${CYTX(dataFee)} CYTX`);
	...
	async buildTransaction(txParamsArg: TxSend, debug = false): Promise < BuildTxResult > {
		...
		this.logger.info(`tx ... required data fee: ${CYTX(dataFee)} (${utxos.length} UTXOs)`);// (${CYTX(txParamsArg.fee)}+${CYTX(dataFee)})`);
		//this.logger.verbose(`tx ... final fee: ${CYTX(dataFee+txParamsArg.fee)} (${CYTX(txParamsArg.fee)}+${CYTX(dataFee)})`);
		this.logger.info(`tx ... resulting total: ${CYTX(totalAmount)}`);
		...
		const ts_3 = Date.now();
		const inputs: RPC.TransactionInput[] = tx.inputs.map((input: cryptixcore.Transaction.Input) => {
			if (debug || this.loggerLevel > 0) {
		...
		const ts_4 = Date.now();
		const outputs: RPC.TransactionOutput[] = tx.outputs.map((output: cryptixcore.Transaction.Output) => {
			return {
		...
		//@ ts-ignore
		//const payloadHash = cryptixcore.crypto.Hash.sha256sha256(Buffer.from(payloadStr));
	...
	*/
	...
	setLogLevel(level: string) {
		...
		cryptixcore.setDebugLevel(level?1:0);
	}
}
