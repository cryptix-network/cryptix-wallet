...
class LSDB extends DBInterface{
	...
	backup():void{
		...
		if(!data)
			return
		let ts = Date.now();
		this.LS.setItem("cryptix-wallet-"+ts, data);
		let cache = this.getCache();
		if(cache)
			this.LS.setItem("cryptix-cache-"+ts, cache);
	}
	saveWallet(data:string):void{
		this.LS.setItem("cryptix-wallet", data);
	}

	getWallet():string|false{
		return this.LS.getItem("cryptix-wallet");
	}

	saveCache(cache:string){
		return this.LS.setItem("cryptix-cache", cache);
	}

	getCache():string|undefined{
		return this.LS.getItem("cryptix-cache");
	}
}
...
if(IS_NODE){
	...
	class FileDB extends DBInterface{
		...
		constructor(opt:DBOptions={}){
			super();
			let {fileName="cryptix", folder} = opt;
		...
		getHomeFolder(){
			...
			if(APPDATA) // windows
				return path.join(APPDATA, 'Cryptix');

			if(process.platform == 'darwin')
				return path.join(HOME,'/Library/Application Support/Cryptix/');

			return path.join(HOME, '.cryptix');
		}
...
export class Storage{
	...
	constructor(opt:StorageOpts={}){
		this.logger = CreateLogger("CryptixStorage");
