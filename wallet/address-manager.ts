...
// @ts-ignore
const secp256k1 = cryptixcore.secp256k1;//require('secp256k1-wasm');
...
export class AddressManager extends EventTargetImpl {
	constructor(HDWallet: cryptixcore.HDPrivateKey, network: Network) {
		super();
		this.HDWallet = HDWallet;
		this.network = network;
	}

	private HDWallet: cryptixcore.HDPrivateKey;

	network: Network;

	get all(): Record < string, cryptixcore.PrivateKey > {
		return {
	...
	receiveAddress: {
		...
		current: {
			address: string;
			privateKey: cryptixcore.PrivateKey
		};
		keypairs: Record < string, cryptixcore.PrivateKey > ;
	...
	private deriveAddress(
		...
		//let address1 = new cryptixcore.PublicKey(publicKeys.pubkey, {network:this.network}).toAddress().toString();
		...
		let address = cryptixcore.Address.fromPublicKeyBuffer(xonly, this.network).toString();
